...
def regrid_Geom(work_dir,geom_name,load_name,seed_scale = 1.0,increment='last'):
    ...
    gridCoords_node_initial = gridCoords_node_initial_matrix.reshape((-1,gridCoords_node_initial_matrix.shape[-1]), order = 'F')
    ...
    cornersRVE_idx    = [
        np.argwhere(np.isclose(gridCoords_node_initial, cornersRVE_coords[corner]).all(axis=1)).item() for corner in range(3)]
