...
class ROLLING(DAMASK):
    ...
    def rolling_parameters(self, number_passes, height_reduction, rolling_speed, contact_length, increments, regridding=False):
            ...
            if step<1:
                ...
                if self._regridding:
                    ...
                    map_0to_rg, cells_rg, size_rg, increment_title = rgg.regrid_geom(geom_name, load_name_old,
                                        seed_scale = seed_scale, seeds = 'nan', increment = 'last') # no output
                    ...
                    args="cp %s %s"%(self._regridedrestartfile,f"{self._regrid_geom_file_old}_{load_name_old}.hdf5")
                    subprocess.run(args,shell=True,capture_output=True) 
                    # print('Do: ',args)
                    ...
                    args="DAMASK_grid -g damask.vti -l %s.yaml -r %d > %s.log"%(self._loadfilename,step*self._increments,self._loadfilename)
                    print('Damask command is:',args)
                    subprocess.run(args,shell=True,capture_output=True) 
                    resultfile = 'damask_%s.hdf5'%(self._loadfilename)
